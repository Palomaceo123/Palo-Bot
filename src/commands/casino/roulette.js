...
module.exports = async (client, interaction, args) => {
    ...
    Schema.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, data) => {
        if (data) {
            ...
            let colour = interaction.options.getString('couleur');
            let money = parseInt(interaction.options.getNumber('montant'));

            let random = Math.floor(Math.random() * 37);

            if (!colour || !money) return client.errUsage({ usage: "roulette [couleur] [montant]", type: 'editreply' }, interaction);
