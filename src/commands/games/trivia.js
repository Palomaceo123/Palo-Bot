    ...
    await client.embed({
        ...
        fields: [
            {
                ...
                value: `Vous n'avez que **${convertTime(60000)}** pour deviner la réponse !\n\n${opt}`
            }
        ],
        components: [row],
        type: 'editreply'
        ...
        collector.on('collect', async (trivia) => {
            ...
            if (trivia.customId === winningID) {
                ...
                await client.embed({
                    title: `🕹️・Trivia`,
                    desc: `GG, c'était **${question.options[question.correct]}**. Vous avez donné la bonne réponse en **${time}**.`,
                ...
                if (winningID === id1) {
                    ...
                    if (trivia.customId === id2) {
                        fbtn2.setStyle('DANGER');
                        fbtn3.setStyle('SECONDAIRE');
                        fbtn4.setStyle('SECONDAIRE');
                    } else if (trivia.customId === id3) {
                        fbtn2.setStyle('SECONDAIRE');
                        fbtn3.setStyle('DANGER');
                        fbtn4.setStyle('SECONDAIRE');
                ...
                await client.embed({
                    title: `🕹️・Trivia`,
                    desc: `Plus de chance la prochaine fois ! La bonne réponse était **${question.options[question.correct]}**.`,
                    components: [{ type: 1, components: [fbtn1, fbtn2, fbtn3, fbtn4] }],
                    type: 'editreply'
                }, interaction)
            }
