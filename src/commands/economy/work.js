...
module.exports = async (client, interaction, args) => {
  ...
  Schema2.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, dataTime) => {
    if (dataTime && dataTime.Work !== null && timeout - (Date.now() - dataTime.Work) > 0) {
      ...
      let replies = ['Programmeur', 'Hacker', 'Serveur', 'Aide-serveur', 'Chef', 'Mécanicien']

      let result = Math.floor((Math.random() * replies.length));
      let amount = Math.floor(Math.random() * 100) + 1;

      client.succNormal({ text: `Vous avez travaillé comme ${replies[result]} et gagné : **${client.emotes.economy.coins} $${amount}**`, type: 'editreply' }, interaction);
