...
module.exports = async (client, interaction, args) => {
    ...
    if (userItems) {
        if (userItems.FishingRodUsage >= randrod) {
            ...
            return client.errNormal({ error: "Ta canne à pêche est cassée ! Va en acheter une nouvelle !", type: 'editreply' }, interaction);
        }
    }
    ...
    Schema2.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, dataTime) => {
        if (dataTime && dataTime.Fish !== null && timeout - (Date.now() - dataTime.Fish) > 0) {
            ...
            client.succNormal({ text: `Vous avez pêché et obtenu un(e) ${fishToWin}`, type: 'editreply' }, interaction);
